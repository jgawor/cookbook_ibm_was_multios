  ...
  if @current_resource.sdkCommandDefault
    ...
    converge_by("setCommandDefault #{@new_resource}") do
      ...
      when 'linux'
        ...
        cmd = "USER_INSTALL_ROOT='#{new_resource.install_dir}' #{new_resource.install_dir}/bin/managesdk.sh -setCommandDefault -sdkname #{sdk_version}"
        log cmd
  ...
  if @current_resource.sdkNewProfileDefault
    ...
    converge_by("setNewProfileDefault #{@new_resource}") do
      ...
      when 'linux'
        sdk_version = getsdkversion
        log "sdk_version: #{sdk_version}"
        cmd = "USER_INSTALL_ROOT='#{new_resource.install_dir}' #{new_resource.install_dir}/bin/managesdk.sh -setNewProfileDefault -sdkname #{sdk_version}"
        log cmd
        cmd_out = run_shell_cmd(cmd, new_resource.admin_user)
        log cmd_out.stdout
      end
...
def getsdkversion
  ...
  when 'linux'
    ...
    cmd_available = "USER_INSTALL_ROOT='#{new_resource.install_dir}' #{new_resource.install_dir}/bin/managesdk.sh -listAvailable | grep #{java_v}"
...
def check_sdk?(option)
  ...
  when 'linux'
    sdk_version = getsdkversion
    cmd_check_sdk = "USER_INSTALL_ROOT='#{new_resource.install_dir}' #{new_resource.install_dir}/bin/managesdk.sh -#{option}"
  end
